...
| `-f ./manifest.yaml` — local path works but sources are still fetched from GitHub | Passing a local filesystem path to `-f` is now fully supported. Relative and absolute paths are detected automatically and read from disk — no `resourceMappings` config needed. Sources referenced inside the manifest are still routed normally: absolute GitHub URLs go to the configured GitHub host; relative paths (e.g. `./README.md`) are resolved relative to the manifest directory and also read from disk. <!-- verified: cmd/app/exec.go IsLocalPath detection; pkg/registry/repositoryhost/local_path.go LocalPath host --> |

...
type taskQueue struct {
	...
	workFunc WorkerFunc
	// failFast controls the behavior of this TaskQueue upon errors. If set to true, it will quit
	// further processing upon the first error that occurs. For fault tolerant applications
	// use false.
	failFast bool
	// tracks the number tasks to wait for
	wg *sync.WaitGroup
	// tasks is the queue for tasks picked up by the workers in this TaskQueue. The AddTask
...
// returns true if the task is added and false if it is skipped
// (e.g. if the taskQueue is stopped or failFast situation)
func (jq *taskQueue) AddTask(task interface{}) bool {
	defer func() {
		if recover() != nil {
...
// GetErrorList returns the errors, occurred during task processing
func (jq *taskQueue) GetErrorList() *multierror.Error {
	return jq.errList
}

// GetErrorList returns the errors, occurred during task processing
func (jq *taskQueue) Name() string {
	return jq.id
}

// GetProcessedTasksCount returns the processed tasks count
func (jq *taskQueue) GetProcessedTasksCount() int {
	return int(jq.tc)
}

// GetWaitingTasksCount returns waiting tasks count
func (jq *taskQueue) GetWaitingTasksCount() int {
	return len(jq.tasks)
}
...
// triggers taskQueue stop if failFast is true
func (jq *taskQueue) appendError(err error) {
	jq.mux.Lock()
	defer jq.mux.Unlock()
...
func (jq *taskQueue) shouldProcess() bool {
	jq.mux.Lock()
	defer jq.mux.Unlock()

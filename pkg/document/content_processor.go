...
type DownloadScheduler interface {
	...
	Schedule(Source string, Target string, Referer string, Reference string) error
}
...
type NodeContentProcessor interface {
	...
	Process(ctx context.Context, buffer *bytes.Buffer, reader readers.Reader, node *manifest.Node) error
}

// NewNodeContentProcessor creates NodeContentProcessor objects
func NewNodeContentProcessor(resourcesRoot string, downloadJob DownloadScheduler, validator Validator, rh resourcehandlers.Registry, hugo hugo.Hugo) NodeContentProcessor {
...
func (c *nodeContentProcessor) Process(ctx context.Context, b *bytes.Buffer, r readers.Reader, n *manifest.Node) error {
